...
uninstaller=/usr/local/sbin/lintex-webhook-uninstall
...
for file in "$tmpfiles" "$updater" "$launcher" "$uninstaller" "$sudoers"; do
  if [[ -f "$file" ]]; then
...
install -o root -g root -m 755 "$workdir/uninstall.sh" "$uninstaller"
